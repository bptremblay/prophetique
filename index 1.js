// TODO: make getItem consume words until they're gone
// First time we getItem, convert it to a hash
// Then, someword picks it from there.
// When the hash value is empty, we start over.
...
function genSimple(vocH) {
    ...
    var cft;
    cft = '';
    var tense = randomInteger(2);
    // ORIGINAL: //  put simple(random(2), random(2), random(4), vocH) after cft
    cft += (simple(randomInteger(2), randomInteger(2), tense, vocH));
    if (randomInteger(3) === 1) {
        cft += ' ' + somePreposition(vocH) + ' ' + someIndirectObject(vocH) + ' ';
    }
    ...
    if (tense === 2 && randomInteger(2) === 1) {
        cft = somePrefix(vocH) + ', ' + cft;
    }
    return makeSent(cft);
...
function someIndirectObject(holder) {
    var trand;
...
function somePreposition(holder) {
    var trand;
    // ORIGINAL: //  put item 6 of holder into temp
    var temp;
    temp = getItem(holder, 11 - 1);
    // ORIGINAL: //  repeat
    while (true) {
...
function somePrefix(holder) {
    var trand;
    // ORIGINAL: //  put item 6 of holder into temp
    var temp;
    temp = getItem(holder, 12 - 1);
    // ORIGINAL: //  repeat
    while (true) {
